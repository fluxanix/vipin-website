...
const TaskCard: React.FC<TaskCardProps> = ({ task, onDragStart, onDragEnd, onStatusChange }) => {
  ...
  return (
    <div
      ...
      className={`task-card p-3 sm:p-4 bg-card rounded-md border border-border shadow-sm hover:shadow-md transition-all duration-200 h-36 sm:h-44 flex flex-col ${isDragging ? 'dragging' : ''}`}
    >
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 mb-2 sm:mb-3">
          <h5 className="font-medium mb-1 sm:mb-2 text-foreground text-xs sm:text-sm leading-tight line-clamp-2">{task.title}</h5>
